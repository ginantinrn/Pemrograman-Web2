            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered" style="border: 1px solid #fff;">
                        ...
                        <tbody>
                            @foreach ($dataPegawai as $nama)
                                <tr style="color: black">
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $nama->nama }}</td>
                                    <td>{{ $nama->nip }}</td>
                                    <td>{{ $nama->posisi_pekerjaan_id}}</td>
                                    <td>{{ $nama->kontak }}</td>
                                    <td>{{ $nama->alamat }}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="/pegawai/view/{{ $nama->id }}" class="btn btn-primary btn-sm btn-action"><i
                                                class="fas fa-eye"></i> View</a>
                                            <a href="/pegawai/edit/{{ $nama->id }}" class="btn btn-warning btn-sm btn-action"><i
                                                    class="fas fa-edit"></i> Edit</a>
                                            <form id="delete-form-{{ $nama->id }}"
                                                action="/pegawai/delete/{{ $nama->id }}" method="POST">
                                                @csrf
                                                @method('DELETE')
